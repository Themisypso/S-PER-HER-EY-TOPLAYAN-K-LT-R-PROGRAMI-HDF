...
import { Heart, MessageSquare, Star, CheckCircle, Plus, Edit3, Bookmark, Gamepad2, BookOpen, Tv, Globe } from 'lucide-react'
...
export function ActivityFeed({ filter = 'following', targetUserId, take, hideHeader }: ActivityFeedProps) {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (response.success && response.data) {
                    ...
                    let newActivities = currentCursor ? [...activities, ...fetchedActivities] : fetchedActivities
    ...
    const getActivityActionText = (activity: Activity) => {
        ...
        if (activity.type === 'ADDED_MEDIA') return t('added_library')
        if (activity.type === 'STATUS_CHANGE') {
            const statusMap: Record<string, string> = {
                'WATCHING': t('now_watching'),
                'COMPLETED': t('completed'),
                'PLANNED': t('plans_watch'),
                'DROPPED': t('dropped_status')
            }
        ...
        if (activity.type === 'QUOTE') return t('added_quote')
        if (activity.type === 'LIST_CREATED') return t('created_list')
        return t('interacted_with')
    }
                    ...
                    <div className="flex-1 min-w-0">
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-2 mt-1 mb-2">
                                    ...
                                    <div className="flex flex-col">
                                        ...
                                        {activity.wasTranslated && activity.detectedLang && (
                                            <div className="flex items-center gap-1 text-[10px] text-text-secondary/70 italic mt-0.5">
                                                <Globe size={10} />
                                                <span>{t('translated_from', { lang: activity.detectedLang.toUpperCase() })}</span>
                                            </div>
                                        )}
                                    </div>
                                </div>
                        ...
                        {activity.type === 'FAVORITED_PERSON' && activity.referenceId && (
                            <Link href={`/person/${activity.referenceId}`} className="mt-2 inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-bg-secondary/50 border border-border/50 hover:bg-bg-secondary transition-colors text-xs font-semibold text-accent-pink">
                                <Heart size={14} className="fill-accent-pink text-accent-pink" /> {t('view_profile')}
                            </Link>
                        )}
                    </div>
                </GlassCard>
